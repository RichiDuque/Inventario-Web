...
<div class="row">
	<div class="col-md-12">
		<h1>Bienvenido a Inventario Frito Lay</h1>
</div>
</div>
  <div class="row">
...
foreach($products as $product):
	$q=OperationData::getQYesF($product->id);
	?>
	<?php if($q<=$product->cant_caja):?>
	<tr class="<?php if($q==0){ echo "danger"; }else if($q<=$product->cant_caja/2){ echo "danger"; } else if($q<=$product->cant_caja){ echo "warning"; } ?>">
		<td><?php echo $product->id; ?></td>
		<td><?php echo $product->name; ?></td>
		<td><?php echo ($q*$product->unit)/$product->description; ?></td>
		<td>
		<?php if($q==0){ echo "<span class='label label-danger'>No hay existencias.</span>";}else if($q<=$product->cant_caja/2){ echo "<span class='label label-danger'>Quedan muy pocas existencias.</span>";} else if($q<=$product->cant_caja){ echo "<span class='label label-warning'>Quedan pocas existencias.</span>";} ?>
		</td>
	</tr>
<?php endif;?>
<?php
